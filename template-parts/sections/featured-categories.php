...
<section class="max-w-7xl mx-auto px-4 py-14 lg:py-20" aria-labelledby="categories-heading">
	<div class="flex items-end justify-between mb-8">
		<div>
			<p class="text-xs font-bold tracking-widest uppercase text-brand-green mb-2"><?php esc_html_e( 'Categories', 'pemu' ); ?></p>
			<h2 id="categories-heading" class="font-display font-extrabold text-3xl lg:text-4xl text-[var(--color-text)] leading-tight">
				<?php esc_html_e( 'Shop by Category', 'pemu' ); ?>
			</h2>
			<p class="text-[var(--color-text-muted)] mt-2"><?php esc_html_e( 'Built for results. Trusted by Kenya\'s strongest.', 'pemu' ); ?></p>
		</div>
	...
	<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5">
		...
		foreach ( $categories as $cat ) :
			...
			// First category gets a double-wide slot on desktop
		...
		<a href="<?php echo esc_url( $cat_url ); ?>"
		   class="group relative aspect-[4/5] <?php echo esc_attr( $extra_class ); ?> rounded-2xl overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 focus-visible:ring-2 focus-visible:ring-brand-green focus-visible:outline-none">
			...
			<!-- Gradient background -->
			<div class="absolute inset-0 bg-gradient-to-br <?php echo esc_attr( $gradient ); ?> transition-transform duration-500 group-hover:scale-105"></div>
			<!-- Darkening overlay -->
			...
			<!-- Emoji decoration -->
			<div class="absolute top-4 right-4 text-4xl lg:text-5xl transition-transform duration-300 group-hover:scale-110 select-none" aria-hidden="true">
				<?php echo esc_html( $emoji ); ?>
			</div>

			<!-- Text content -->
			<div class="absolute bottom-0 left-0 right-0 p-4 lg:p-5 text-white">
				<p class="text-[11px] font-semibold uppercase tracking-wider opacity-80 leading-none"><?php echo esc_html( $count_txt ); ?></p>
				<h3 class="font-display font-bold text-xl mt-1 leading-tight"><?php echo esc_html( $cat->name ); ?></h3>
				<span class="inline-flex items-center gap-1 mt-2 text-xs font-semibold opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-200">
