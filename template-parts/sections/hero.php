 ...
 * Pulls content from Theme Customizer settings.
 */
...
<section class="relative overflow-hidden min-h-[85vh] flex items-center" aria-labelledby="hero-heading">
	...
	<div class="absolute inset-0 -z-10" aria-hidden="true">
		...
		<img src="<?php echo esc_url( $hero_image ); ?>"
		     ...
		     loading="eager"
		     fetchpriority="high">
		<div class="absolute inset-0 bg-[var(--color-bg)]/80 dark:bg-[var(--color-bg)]/90"></div>
		<?php else : ?>
		<div class="absolute inset-0 bg-gradient-to-br from-brand-light via-[var(--color-bg)] to-[var(--color-bg)] dark:from-[#0F1923] dark:via-[#142231] dark:to-[#0F1923]"></div>
		<?php endif; ?>
		<!-- Decorative blobs -->
		...
		<div class="absolute inset-0 opacity-[0.03] dark:opacity-[0.06]"
		     style="background-image: linear-gradient(var(--color-text) 1px,transparent 1px),linear-gradient(90deg,var(--color-text) 1px,transparent 1px);background-size:60px 60px;"></div>
	</div>
	...
	<div class="max-w-7xl mx-auto px-4 py-16 lg:py-24 grid lg:grid-cols-2 gap-10 lg:gap-16 items-center w-full">
		...
		<div class="max-w-xl">
			...
			<h1 id="hero-heading" class="font-display font-extrabold text-4xl sm:text-5xl lg:text-6xl xl:text-7xl leading-[1.05] tracking-tight">
				<?php if ( $headline_2 ) : ?>
				<span class="text-[var(--color-text)] block"><?php echo esc_html( $headline_1 ); ?></span>
				<span class="text-brand-green block mt-1"><?php echo esc_html( $headline_2 ); ?></span>
				<?php else : ?>
				<span class="text-[var(--color-text)]"><?php echo esc_html( $headline ); ?></span>
				<?php endif; ?>
			</h1>
			...
			<div class="mt-10 flex items-center gap-4">
				<div class="flex -space-x-2.5" aria-hidden="true">
					...
					<div class="w-9 h-9 rounded-full <?php echo esc_attr( $a['bg'] ); ?> border-2 border-[var(--color-bg)] flex items-center justify-center text-white text-xs font-bold shrink-0">
				...
				<div>
					...
					<p class="text-sm text-[var(--color-text-muted)]"><?php echo esc_html( $social_text ); ?></p>
				</div>
			</div>
		</div>
		...
		<div class="relative hidden lg:block" aria-hidden="true">
			<div class="relative mx-auto w-full max-w-md aspect-square">
				...
				<div class="relative w-full h-full rounded-3xl overflow-hidden bg-gradient-to-br from-brand-navy to-brand-green p-1 shadow-2xl shadow-brand-green/20">
					<div class="w-full h-full rounded-3xl bg-[var(--color-surface)] flex items-center justify-center">
						<?php
						// Try to show a featured product image, fallback to emoji
						$featured = wc_get_products( [ 'featured' => true, 'limit' => 1, 'status' => 'publish' ] );
						if ( ! empty( $featured ) ) :
							$fp = $featured[0];
				...
				<div class="absolute -bottom-5 -left-8 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl px-4 py-3 shadow-xl flex items-center gap-3 animate-float">
					...
					<div>
						<p class="font-bold text-sm text-[var(--color-text)] leading-tight"><?php esc_html_e( 'Same-Day Delivery', 'pemu' ); ?></p>
						<p class="text-[11px] text-[var(--color-text-muted)]"><?php esc_html_e( 'Nairobi & Mombasa', 'pemu' ); ?></p>
					</div>
				</div>
				<!-- Floating card: Secure -->
				<div class="absolute -top-5 -right-8 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl px-4 py-3 shadow-xl flex items-center gap-3">
					...
					<div>
						<p class="font-bold text-sm text-[var(--color-text)] leading-tight"><?php esc_html_e( 'M-Pesa Secured', 'pemu' ); ?></p>
						<p class="text-[11px] text-[var(--color-text-muted)]"><?php esc_html_e( 'Pay on delivery', 'pemu' ); ?></p>
					</div>
				</div>
				<!-- Floating card: Rating -->
				<div class="absolute top-1/2 -right-12 -translate-y-1/2 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl px-3 py-2.5 shadow-xl">
					...
					<p class="text-xs font-bold text-[var(--color-text)] mt-0.5">4.9/5</p>
					<p class="text-[10px] text-[var(--color-text-muted)]">10k+ reviews</p>
				</div>
			</div>
		</div>
