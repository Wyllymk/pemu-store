 ...
 * Pemu override: styled account sidebar nav.
 * @version 2.6.0
 */
...
?>
<nav class="woocommerce-MyAccount-navigation bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl overflow-hidden"
	...
	<ul class="list-none p-2 m-0 space-y-0.5">
		...
		?>
		<li class="mt-2 pt-2 border-t border-[var(--color-border)]">
		...
		<li class="<?php echo wc_get_account_menu_item_classes( $endpoint ); ?>">
			<a href="<?php echo esc_url( wc_get_account_endpoint_url( $endpoint ) ); ?>"
			   class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-colors group <?php echo $is_active ? 'bg-brand-green/10 text-brand-green' : 'text-[var(--color-text)] hover:bg-[var(--color-bg-muted)] hover:text-brand-green'; ?>">
