...
if ( ! $in_stock ) {
	...
	$badge_class = 'bg-slate-500 dark:bg-slate-400';
} elseif ( $is_featured ) {
...
<article class="group relative bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden hover:shadow-xl hover:shadow-black/5 hover:-translate-y-1 transition-all duration-300 flex flex-col">

	<!-- Image area -->
	<a href="<?php echo esc_url( $permalink ); ?>"
	   class="relative block aspect-square bg-gradient-to-br from-gray-100 dark:from-slate-800 to-white dark:to-slate-900 overflow-hidden flex items-center justify-center"
	   tabindex="-1" aria-hidden="true">
		<?php echo $img_html; ?>
		<?php if ( ! $in_stock ) : ?>
		<div class="absolute inset-0 bg-white/60 dark:bg-slate-800/60 flex items-center justify-center">
			<span class="text-xs font-bold text-slate-500 dark:text-slate-400"><?php esc_html_e( 'Out of Stock', 'pemu' ); ?></span>
		</div>
		<?php endif; ?>
	</a>
	...
	<div class="flex flex-col flex-1 p-4 gap-2">
		...
		<p class="text-[10px] font-bold tracking-widest uppercase text-slate-500 dark:text-slate-400"><?php echo esc_html( $brand ); ?></p>
		<?php endif; ?>
		...
		<div class="flex items-center gap-1.5">
			<div class="flex" aria-label="<?php echo esc_attr( sprintf( __( 'Rated %.1f out of 5', 'pemu' ), $avg_rating ) ); ?>">
				<?php for ( $s = 1; $s <= 5; $s++ ) : ?>
				<svg class="w-3 h-3 <?php echo $s <= $avg_rating ? 'text-amber-400' : 'text-slate-200 dark:text-slate-700'; ?>" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
				<?php endfor; ?>
			</div>
			<span class="text-xs text-slate-500 dark:text-slate-400">(<?php echo esc_html( $review_count ); ?>)</span>
		</div>
		<?php endif; ?>
		...
		<div class="flex items-end justify-between mt-auto pt-2">
			...
			$atc_href  = $is_simple ? esc_url( add_query_arg( 'add-to-cart', $product->get_id(), $product->get_permalink() ) ) : $permalink;
			$atc_class = 'shrink-0 w-9 h-9 rounded-full bg-brand-green hover:bg-brand-green-dark !text-white flex items-center justify-center transition-colors duration-200 shadow-sm';
			if ( $is_simple ) {
				$atc_class .= ' add_to_cart_button ajax_add_to_cart product_type_simple';
			}
			?>
			<a href="<?php echo esc_url( $atc_href ); ?>"
			   <?php if ( $is_simple ) : ?>
			   ...
			   rel="nofollow"
			   <?php endif; ?>
			   aria-label="<?php echo esc_attr( $is_simple ? sprintf( __( 'Add %s to cart', 'pemu' ), $name ) : sprintf( __( 'View %s options', 'pemu' ), $name ) ); ?>"
			   class="<?php echo esc_attr( $atc_class ); ?>">
