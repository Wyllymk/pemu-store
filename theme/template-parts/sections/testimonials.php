...
<section class="max-w-7xl mx-auto px-4 py-14 lg:py-20" aria-labelledby="reviews-heading">
	<div class="text-center mb-12">
		...
		<h2 id="reviews-heading" class="font-display font-extrabold text-3xl lg:text-4xl text-[#1a1a2e] dark:text-[#e8edf2]">
		...
		<div class="mt-4 inline-flex items-center gap-3">
			...
			<span class="font-display font-bold text-xl text-[#1a1a2e] dark:text-[#e8edf2]">4.9</span>
			<span class="text-sm text-slate-500 dark:text-slate-400"><?php esc_html_e( 'from 2,400+ reviews', 'pemu' ); ?></span>
		</div>
	</div>

	<div class="columns-1 sm:columns-2 lg:columns-3 gap-5 space-y-5">
		<?php foreach ( $reviews as $r ) : ?>
		<div class="break-inside-avoid bg-white dark:bg-[#162333] border border-slate-200 dark:border-[#2a3f52] rounded-2xl p-6 hover:border-brand-green/40 hover:shadow-lg transition-all duration-200">
			<!-- Stars -->
			<div class="flex gap-0.5 mb-3" aria-label="<?php echo esc_attr( sprintf( __( 'Rated %d out of 5', 'pemu' ), $r['rating'] ) ); ?>">
				<?php for ( $s = 1; $s <= 5; $s++ ) : ?>
				<svg class="w-3.5 h-3.5 <?php echo $s <= $r['rating'] ? 'text-amber-400' : 'text-slate-200 dark:text-[#2a3f52]'; ?>" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
				<?php endfor; ?>
			</div>

			<p class="font-bold text-sm text-[#1a1a2e] dark:text-[#e8edf2] mb-1"><?php echo esc_html( $r['title'] ); ?></p>
			<p class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">&ldquo;<?php echo esc_html( $r['body'] ); ?>&rdquo;</p>

			<!-- Reviewer -->
			<div class="flex items-center gap-3 mt-4 pt-4 border-t border-slate-200 dark:border-[#2a3f52]">
				...
				<div>
					<p class="font-semibold text-sm text-[#1a1a2e] dark:text-[#e8edf2] leading-none"><?php echo esc_html( $r['name'] ); ?></p>
