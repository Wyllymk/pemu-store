...
<main id="main-content" class="min-h-screen bg-gray-100 dark:bg-slate-800">
    <div class="max-w-6xl mx-auto px-4 py-8">
        ...
        <h1 class="font-display font-extrabold text-3xl text-slate-800 dark:text-slate-200 mb-8">
            ...
            <span
                class="text-lg font-normal text-slate-500 dark:text-slate-400 ml-2">(<?php echo WC()->cart->get_cart_contents_count(); ?>
                items)</span>
            <?php endif; ?>
        </h1>
      ...
      ?>
                    ...
                    <div
                        class="<?php echo esc_attr($row_class); ?> bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 sm:p-5 flex gap-4 transition-all duration-200 hover:border-brand-green/30">

                        <!-- Thumbnail -->
                        <div
                            class="w-20 h-20 sm:w-24 sm:h-24 shrink-0 rounded-xl overflow-hidden bg-gradient-to-br from-gray-100 dark:from-slate-800 to-white dark:to-slate-900 flex items-center justify-center">
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0 flex-1">
                                    ...
                                    <a href="<?php echo esc_url($product_permalink); ?>"
                                        class="font-semibold text-sm !text-slate-800 dark:!text-white hover:text-brand-green transition-colors leading-snug block line-clamp-2">
                                        <?php echo wp_kses_post($product_name); ?>
                                    </a>
                                    <?php else: ?>
                                    <span
                                        class="font-semibold text-sm text-slate-800 dark:text-slate-200 leading-snug block"><?php echo wp_kses_post($product_name); ?></span>
                                    <?php endif; ?>

                                    <?php echo wc_get_formatted_cart_item_data($cart_item); // phpcs:ignore ?>

                                    <?php if ($_product->get_sku()): ?>
                                    <span class="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 block">SKU:
                                        <?php echo esc_html($_product->get_sku()); ?></span>
                                    <?php endif; ?>
                                </div>
            ...
            ?>
                                <a href="<?php echo $remove_url; // phpcs:ignore ?>"
                                    class="shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-slate-600 dark:text-slate-300 hover:text-red-500 hover:bg-red-500/10 transition-colors border border-slate-200 dark:border-slate-600 hover:border-red-200"
                            ...
                            <div class="flex items-end justify-between mt-3 gap-3 flex-wrap">
                                <div
                                    class="quantity inline-flex items-center border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden bg-gray-100 dark:bg-slate-800">
                                    <button type="button"
                                        class="w-9 h-10 flex items-center justify-center shrink-0 bg-none border-none text-base font-bold text-slate-500 dark:text-slate-400 cursor-pointer transition-colors hover:text-brand-green hover:bg-white dark:hover:bg-slate-800 leading-none pemu-qty-btn minus"
                                        data-action="minus" aria-label="Decrease quantity">−</button>
                                    <input type="number"
                                    ...
                                    <button type="button"
                                        class="w-9 h-10 flex items-center justify-center shrink-0 bg-none border-none text-base font-bold text-slate-500 dark:text-slate-400 cursor-pointer transition-colors hover:text-brand-green hover:bg-white dark:hover:bg-slate-800 leading-none pemu-qty-btn plus"
                                        data-action="plus" aria-label="Increase quantity">+</button>
                                </div>

                                <div class="text-right">
                                    <p class="text-[10px] text-slate-500 dark:text-slate-400">
                                        <?php echo wp_kses_post($product_price); ?> each</p>
                                    <p class="font-display font-bold text-brand-green text-base">
                                        <?php echo $product_subtotal; // phpcs:ignore ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5"
                        ...
                        <div class="flex flex-wrap gap-3 items-start">
                            <div class="flex-1 min-w-0">
                                ...
                                <div x-show="couponOpen" x-cloak x-transition class="mt-3 flex gap-2">
            ...
            <?php else: ?>
            <div
                class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-12 text-center">
                <div
                    class="w-20 h-20 rounded-full bg-gray-100 dark:bg-slate-800 flex items-center justify-center mx-auto mb-5">
                ...
                <p class="font-display font-bold text-xl text-slate-800 dark:text-slate-200 mb-2">
                    <?php esc_html_e('Your cart is empty','woocommerce'); ?></p>
                <p class="text-slate-500 dark:text-slate-400 text-sm mb-7">Looks like you haven't added any products
                    yet.</p>
                <a href="<?php echo esc_url(wc_get_page_permalink('shop')); ?>"
                    ...
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path d="M3 9h18l-2 11H5zM8 9V6a4 4 0 118 0v3" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
