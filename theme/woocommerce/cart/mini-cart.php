...
if ( ! WC()->cart->is_empty() ) : ?>
	...
	<div class="flex-1 overflow-y-auto overscroll-contain px-5 py-4 space-y-4"
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) :
			...
			<div class="<?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item pemu-cart-item', $cart_item, $cart_item_key ) ); ?> flex gap-3 pb-4 border-b border-slate-200 dark:border-slate-700 last:border-0 last:pb-0"
			     role="listitem">
				<div class="w-20 h-20 shrink-0 rounded-xl overflow-hidden bg-gray-100 dark:bg-slate-800 flex items-center justify-center">
				...
				<div class="flex-1 min-w-0">
					<div class="flex items-start justify-between gap-2">
						<div class="min-w-0">
							<?php if ( ! $product_permalink ) : ?>
							<span class="font-semibold text-sm text-slate-800 dark:text-slate-200 leading-snug"><?php echo wp_kses_post( $product_name ); ?></span>
							<?php else : ?>
							<a href="<?php echo esc_url( $product_permalink ); ?>" class="font-semibold text-sm text-slate-800 dark:text-slate-200 leading-snug hover:text-brand-green transition-colors line-clamp-2">
							...
							<div class="flex items-center gap-2 mt-2">
								<div class="inline-flex items-center border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
									<button class="px-2.5 py-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-brand-green hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors"
									...
									<span class="px-3 py-1.5 text-sm font-semibold text-slate-800 dark:text-slate-200 tabular-nums min-w-[32px] text-center"><?php echo (int) $cart_item['quantity']; ?></span>
									<button class="px-2.5 py-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-brand-green hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors"
	...
	<div class="px-5 py-4 border-t border-slate-200 dark:border-slate-700 space-y-4 bg-gray-100 dark:bg-slate-800">
		...
		<a href="<?php echo esc_url( pemu_whatsapp_url( pemu_cart_whatsapp_message() ) ); ?>"
		   ...
		   class="flex items-center justify-center gap-2 w-full py-3 rounded-xl border-2 border-green-500 text-green-500 font-semibold hover:bg-green-500 hover:text-white transition-all duration-200 text-sm">
...
<?php else : ?>
	<div class="flex flex-col items-center justify-center flex-1 px-8 py-16 text-center">
		...
		<p class="font-display font-bold text-lg text-slate-800 dark:text-slate-200 mb-2"><?php esc_html_e( 'Your cart is empty', 'pemu' ); ?></p>
		<p class="text-sm text-slate-500 dark:text-slate-400 mb-6"><?php esc_html_e( 'Add some supplements to get started!', 'pemu' ); ?></p>
